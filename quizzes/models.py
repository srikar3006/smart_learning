...
class Quiz(models.Model):
    ...
    rhyme = models.OneToOneField(Rhyme, on_delete=models.CASCADE, related_name="quiz")
    title = models.CharField(max_length=150)
    passing_score_pct = models.PositiveIntegerField(default=60)

    def clean(self):
        if not 0 <= self.passing_score_pct <= 100:
            raise ValidationError({"passing_score_pct": "Passing score must be between 0 and 100."})
...
class Question(models.Model):
    ...
    text = models.CharField(max_length=255, help_text="Keep it short and simple for early readers.")
    ...
    image_prompt = models.ImageField(upload_to="quiz/image_prompts/", blank=True, null=True)
...
class Choice(models.Model):
    ...
    image = models.ImageField(upload_to="quiz/choice_images/", blank=True, null=True)
    ...
    class Meta:
        ordering = ["order", "id"]
        constraints = [
            models.UniqueConstraint(fields=["question", "order"], name="unique_question_choice_order"),
        ]

    def clean(self):
        if not self.text and not self.emoji and not self.image:
            raise ValidationError("A choice needs text, an emoji, or an image.")
...
class QuizAnswer(models.Model):
    ...
    def __str__(self):
        return f"{self.attempt} – {self.question} – {'correct' if self.is_correct else 'wrong'}"
